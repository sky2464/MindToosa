...
1. [x] Create app + repo.
2. [x] Implement Auth.js: `auth.ts` + `/api/auth` route. [page:4]
3. [x] Add `/today` page with mock plan display.
4. [x] Add Zod schemas for Space/Goal/Task/DailyPlan.

Sprint B (25 min) — real

1. [x] Create Supabase tables.
2. [x] Implement `/api/spaces` and `/api/tasks` with server Supabase client.
3. [x] Implement `POST /api/plan/daily` returning mocked JSON.
4. [x] Implement `POST /api/plan/apply` writing tasks.
...
## 15) Lessons Learned & Setup Quirks

1. **Google OAuth**: `next-auth` requires explicit `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` in `auth.ts` provider config; auto-detection from env vars can be flaky.
2. **Database Init**: Migration scripts (e.g., `supabase/migrations/0000_init.sql`) must be run manually in Supabase SQL Editor for new projects.
3. **User ID Type**: The app uses Email as `user_id` when using Google Auth. Zod schemas must use `z.string()` instead of `.uuid()` for `user_id`.
