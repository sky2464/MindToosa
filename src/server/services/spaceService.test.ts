...
describe("spaceService", () => {
  ...
  describe("createSpace", () => {
    ...
    it("should throw ValidationError on invalid data", async () => {
      ...
      await expect(spaceService.createSpace(userId, "")).rejects.toThrow(ValidationError);
    });
  });

  describe("updateSpace", () => {
    it("should validate UUID", async () => {
      await expect(spaceService.updateSpace(userId, "invalid", {})).rejects.toThrow(
        ValidationError
      );
    });
  ...
  describe("archiveSpace", () => {
    it("should validate UUID", async () => {
      await expect(spaceService.archiveSpace(userId, "invalid")).rejects.toThrow(ValidationError);
    });
