...
describe("taskService", () => {
  ...
  describe("getTasks", () => {
    ...
    it("should validate inputs if applicable", async () => {
      ...
      await expect(taskService.getTasks(userId, { spaceId: "invalid" })).rejects.toThrow(
        ValidationError
      );
    });
  });
  ...
  describe("createTask", () => {
    ...
    it("should throw ValidationError on invalid data", async () => {
      await expect(taskService.createTask(userId, { title: "" })).rejects.toThrow(ValidationError);
    });
  });

  describe("updateTask", () => {
    it("should validate UUID", async () => {
      await expect(taskService.updateTask(userId, "invalid", {})).rejects.toThrow(ValidationError);
    });
    ...
    it("should update task successfully", async () => {
      ...
      mockSingle
        .mockResolvedValueOnce({
          data: { id: taskId, title: "Old Title", user_id: userId, space_id: spaceId },
          error: null,
        }) // 1. Initial fetch
  ...
  describe("deleteTask", () => {
    it("should validate UUID", async () => {
      await expect(taskService.deleteTask(userId, "invalid")).rejects.toThrow(ValidationError);
    });
  });
});
