...
export default function SettingsClient({ userEmail }: SettingsClientProps) {
  ...
  useEffect(() => {
    ...
    import("@/lib/apiClient").then(({ apiClient }) => {
      apiClient
        .get<typeof settings>("/api/settings")
        .then((data) => {
          setSettings(data);
  ...
  return (
    <div className="mx-auto min-h-screen max-w-2xl p-4 pb-24 sm:p-6 lg:p-8">
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-2xl border border-zinc-800 bg-zinc-900/30 p-6">
          <h2 className="mb-4 flex items-center gap-2 text-sm font-bold tracking-widest text-zinc-400 uppercase">
        ...
        <section className="rounded-2xl border border-zinc-800 bg-zinc-900/30 p-6">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="flex items-center gap-2 text-sm font-bold tracking-widest text-zinc-400 uppercase">
              Preferences
            </h2>
            {loadingSettings && (
              <span className="animate-pulse text-xs text-zinc-600">Loading...</span>
            )}
          </div>
          ...
          <div className="space-y-6">
            {/* Theme */}
            <div>
              <label className="mb-2 block text-xs font-bold text-zinc-500 uppercase">Theme</label>
              <div className="flex gap-2 rounded-xl bg-zinc-900/50 p-1">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label
                  ...
                  className="mb-2 block text-xs font-bold text-zinc-500 uppercase"
                >
                  Start Time
                </label>
                <input
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative h-6 w-12 rounded-full transition-colors ${
                  settings.notifications_enabled ? "bg-emerald-500" : "bg-zinc-700"
                }`}
                aria-label={
                  settings.notifications_enabled ? "Disable Notifications" : "Enable Notifications"
                }
              >
                <div
            ...
            <button
              ...
              disabled={saving}
              className="w-full rounded-xl bg-indigo-600 py-2.5 font-bold text-white shadow-lg shadow-indigo-600/20 transition-all hover:bg-indigo-500 disabled:opacity-50"
            >
        ...
        <section className="rounded-2xl border border-zinc-800 bg-zinc-900/30 p-6">
          ...
          <p className="mb-4 text-sm text-zinc-500">
            Your data is stored securely in Supabase. Only the minimum context is sent to the AI
            model when planning is enabled.
          </p>
          <button
            disabled
