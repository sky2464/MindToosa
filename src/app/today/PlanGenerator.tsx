...
export default function PlanGenerator({ spaceId }: PlanGeneratorProps) {
  ...
  const generatePlan = async () => {
    ...
    try {
      const data = await apiClient.post<GeneratedPlan>("/api/plan/daily", {
        ...
        notes:
          notes +
          (rebootMode ? ` [REBOOT MODE: energy=${energyLevel}]` : ` [energy=${energyLevel}]`),
      });
      setPlan(data);
    } catch (err) {
  ...
  const EnergyIcon =
    energyLevel === "low" ? BatteryLow : energyLevel === "medium" ? BatteryMedium : Battery;

  if (!open) {
    return (
      <button
        onClick={() => setOpen(true)}
        className="flex items-center gap-2 rounded-xl border border-indigo-500/30 bg-indigo-600/20 px-4 py-2.5 text-sm font-bold text-indigo-300 transition-all hover:bg-indigo-600/30 hover:text-indigo-200"
      >
            ...
            <>
                ...
                <div className="flex gap-2">
                  {(["low", "medium", "high"] as EnergyLevel[]).map((level) => (
                    <button
                      ...
                      className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl py-2 text-sm font-bold capitalize transition-all ${
                        energyLevel === level
                          ? level === "low"
              ...
              <div>
                ...
                <input
                  ...
                  maxLength={500}
                  className="w-full rounded-xl border border-zinc-700 bg-zinc-900/50 px-4 py-2.5 text-sm text-white placeholder-zinc-600 transition outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/30"
                />
              </div>
            ...
            <div className="space-y-4">
              {applied ? (
                ...
                <>
                        ...
                        {plan.mustDo.map((task, i) => (
                          ...
                          >
                            ...
                            <p className="text-xs text-zinc-500">
                              {task.estimated_minutes}m · {task.micro_steps?.length ?? 0} steps
                            </p>
                          </div>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {plan.optional.map((task, i) => (
                          <div
                            key={i}
                            className="rounded-xl border border-amber-500/10 bg-amber-500/5 px-4 py-3"
                          >
                            <p className="font-medium text-white">{task.title}</p>
                            <p className="text-xs text-zinc-500">
                              {task.estimated_minutes}m · {task.micro_steps?.length ?? 0} steps
                  ...
                  {plan.notes && <p className="text-sm text-zinc-500 italic">{plan.notes}</p>}
