    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-background w-full max-w-lg rounded-xl border border-zinc-800 p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-foreground text-lg font-semibold">AI Breakdown</h2>
              <button
              ...
              <div className="space-y-4">
                ...
                <div className="max-h-[50vh] space-y-2 overflow-y-auto pr-2">
                  {generatedTasks.map((task, idx) => (
                    <div key={idx} className="border-border bg-secondary/30 rounded-lg border p-3">
                      <input
                        aria-label="Task Title"
                        className="text-foreground w-full bg-transparent text-sm font-medium outline-none focus:border-b focus:border-indigo-500"
                        value={task.title || ""}
                        onChange={(e) => {
                ...
                <div className="flex justify-end gap-3 pt-4">
                  <button
                    onClick={() => setIsOpen(false)}
                    className="text-muted-foreground hover:bg-secondary rounded-lg px-4 py-2 font-medium"
                    disabled={isSaving}
                  >
                    Cancel
                  </button>
            ...
            {!isGenerating && !generatedTasks && (
              <div className="py-6 text-center text-red-400">Failed to load subtasks.</div>
            )}
          </div>
        </div>
      )}
