...
export default function TaskEditModal({ task, onClose }: TaskEditModalProps) {
  ...
  return (
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-lg flex-col rounded-2xl border border-white/10 bg-zinc-950 shadow-2xl shadow-indigo-500/10 sm:max-h-[85vh] sm:rounded-3xl">
        {/* Header */}
        <div className="flex shrink-0 items-center justify-between border-b border-white/5 px-4 py-3 sm:px-6 sm:py-4">
          <h2 className="text-sm font-bold text-white sm:text-base">
            Edit Task (Recurrence Added)
          </h2>
          <button
            onClick={onClose}
            className="rounded-lg p-2 text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-zinc-300 sm:p-1.5"
        ...
        {/* Tabs */}
        <div className="flex shrink-0 gap-1 border-b border-white/5 px-4 pt-2">
          {TABS.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-1.5 rounded-t-lg px-3 py-2 text-xs font-bold transition-all ${
                activeTab === tab.id
            ...
            <div className="space-y-5">
              ...
              <div>
                <label
                  ...
                  className="mb-2 block text-xs font-bold tracking-widest text-zinc-500 uppercase"
                >
                  Task Title
                </label>
                <textarea
                  ...
                  rows={2}
                  className="w-full resize-none rounded-xl border border-zinc-700 bg-zinc-900/50 px-4 py-2.5 text-sm text-white placeholder-zinc-600 transition outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/30"
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-1 text-zinc-500 hover:text-zinc-300 sm:p-0"
                  ...
                  <div className="space-y-2">
                    {subtasks.map((st) => (
                      ...
                      >
                        ...
                        />
                        <span
                          className={`flex-1 text-sm ${st.status === "done" ? "text-zinc-500 line-through" : "text-zinc-300"}`}
                        >
                          {st.title}
                        </span>
                    ...
                    {subtasks.length === 0 && (
                      <p className="py-4 text-center text-sm text-zinc-600 italic">
                        No subtasks yet
                      </p>
                    )}
                  </div>
            ...
            <div className="space-y-4">
              <div className="max-h-60 space-y-3 overflow-y-auto">
                {commentsLoading ? (
                  ...
                  <p className="py-8 text-center text-sm text-zinc-600 italic">No comments yet</p>
                ) : (
                  comments.map((c) => (
                    <div
                      key={c.id}
          ...
          {activeTab === "dependencies" && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-zinc-600">
                Tasks listed here must be completed before this task.
              </p>

              {/* Current dependencies */}
              <div className="space-y-2">
                {dependencies.length === 0 && (
                  <p className="py-2 text-sm text-zinc-600 italic">No blocking tasks.</p>
                )}
                {dependencies.map((dep) => (
                  <div
                    key={dep.id}
                    className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-zinc-900/30 px-4 py-2.5"
                  >
                    <span className="flex-1 truncate text-sm text-zinc-300">{dep.title}</span>
                    <button
                      onClick={async () => {
                        try {
                          await apiClient.delete(`/api/tasks/${task.id}/dependencies`, {
              ...
              <div className="space-y-2 border-t border-white/5 pt-3">
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    try {
                      const results = await apiClient.get<Task[]>(
                        `/api/tasks?q=${encodeURIComponent(e.target.value)}`
                      );
                      setDepResults(
                        results.filter(
                ...
                {depResults.map((t) => (
                  <button
                    ...
                    onClick={async () => {
                      try {
                        await apiClient.post(`/api/tasks/${task.id}/dependencies`, {
                          blocking_task_id: t.id,
                        });
                    ...
                    className="flex w-full items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-900/30 px-4 py-2 text-sm text-zinc-400 transition-all hover:border-zinc-600 hover:text-white"
                  >
                    <Plus size={12} />
                    {t.title}
                  </button>
        ...
        <div className="fixed right-0 bottom-0 left-0 mt-auto shrink-0 border-t border-white/5 bg-zinc-950/95 px-4 py-3 backdrop-blur-xl sm:static sm:bg-transparent sm:px-6 sm:py-4 sm:backdrop-blur-none">
          {error && (
