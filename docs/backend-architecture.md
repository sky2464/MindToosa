...
4. Proxy middleware (`proxy.ts`) protects all page routes
...
**Middleware Enforcement (`proxy.ts`):**
...
Session cookie set (secure, HTTP-only)
  ↓
Proxy middleware checks session on next page load
  ↓
...
- **Auth:** src/auth.ts, proxy.ts, src/middleware.ts
